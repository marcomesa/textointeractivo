...
// Valores máximos
	...
	for(var j=0;j<arr.length;j++){
		...
		yy+=arr3[j]*Math.pow(arr4[j],2);
		xy+=arr5[j]*arr4[j]*arr3[j];
	...
	for(var j=0;j<arr.length;j++){
		...
		arr4[j]=parseFloat(arr4[j]);
		x_y+=arr3[j]*(arr5[j]-xmean)*(arr4[j]-ymean);
		cvarx+=arr3[j]*(arr5[j]-xmean)*(arr5[j]-xmean);
		cvary+=arr3[j]*(arr4[j]-ymean)*(arr4[j]-ymean);
		sigmaa+=arr3[j]*(arr4[j]-pend*arr5[j]-orden)*(arr4[j]-pend*arr5[j]-orden);
					}
	...
	controly[i]=A*Math.pow(controlx[i],B);
	}
